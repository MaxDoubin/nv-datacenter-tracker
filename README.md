...
| Finding | Detail |
| --- | --- |
| **Wrong county and wrong entity** in the report to the Legislature | GOED's FY2023 to FY2024 biennial report places Novva's FY2024 award in **Clark County** under **Novva Holdings, LLC**. GOED's own board packet for the same award, identical to the dollar in both tax figures, describes a 300,000 sq ft facility in **Storey County** filed as **Novva Reno, LLC**. |
...
| **An unexplained restatement** | FY2016 post-withdrawal capital investment is `$2,796,566,566` in the FY2010 to FY2022 report (arithmetically exact) and `$2,747,337,806` in the FY2023 to FY2024 report. The difference matches no documented withdrawal. |
...
Program-wide, across every GOED abatement program with completed audits (FY2010 to FY2019), companies delivered
**48% of the capital investment they projected** while exceeding their job projections, at wages well above
what they forecast.

## What the site shows

Nine views, 33 charts, all generated from the CSVs at build time and all theme aware.

- **Maps.** Nevada's 17 counties projected with an Albers equal area conic, shaded by any of five metrics,
  with graduated circles over the top and a dot for each of the 33 data centers the state has counted.
  The circles matter: Storey County holds the largest share of abatement value and is one of the smallest
  counties in Nevada, so shading alone makes the biggest number nearly invisible.
- **A lock in timeline.** Every award drawn from board approval to the fiscal year its term expires,
  against a marker for today. Decisions made in 2015 still bind local revenue into the 2030s.
- **Promised against delivered.** A dumbbell chart of what companies projected versus what GOED found at
  audit, with the delivery ratio on each row.
- **Cost per job.** Every award ranked, with the Tesla large investment award drawn in as a reference line.
- **Composition.** Treemaps by operator, stacked bars splitting sales and use tax from personal property tax,
  donuts for tax mix and generation mix.
- **Resource use.** Area proportional circles for water, so a 352 million gallon figure and a 651,000 gallon
  figure can sit on the same page honestly.
- **A heatmap** of awards by fiscal year and county, where the blanks are the point.

Every chart carries a caption and a link to the document behind it.
...
| [`redemptions.csv`](data/redemptions.csv) | 25 | Abatements *actually* redeemed (City of Reno, FY2017 to FY2024) |
...
2. **Sourced, not computed.** The CSVs hold what documents say. Anything derived, such as abatement per job, implied
   payroll, payroll or cumulative totals, is computed at build time and labelled as such.
...
**No dependencies.** Not "few", but zero, runtime and dev. Node 22.6+ runs the TypeScript directly and
...
- **FY2025 to FY2027 awards.** Board packets exist for approvals this dataset only has via trade press.
